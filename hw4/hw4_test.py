...
def read_test(file):
        ...
        for line in f:
            if count == 0: pass
            elif count <= 10: x.append(line[2:])
            elif count <= 100: x.append(line[3:])
            elif count <= 1000: x.append(line[4:])
            elif count <= 10000: x.append(line[5:])
            elif count <= 100000: x.append(line[6:])
            else: x.append(line[7:])
            count += 1
        return np.array(x)
